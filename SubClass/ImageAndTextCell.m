...
+ (BOOL)prefersTrackingUntilMouseUp{
    ...
    [nc postNotificationName:@"ImageAndTextCellClicked" object:self];
...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
    ...
    _primaryText = [self.delegate primaryTextForCell:data];//[[self.delegate primaryTextForCell:data] retain];
    _auxiliaryText = [self.delegate auxiliaryTextForCell:data];//[[self.delegate auxiliaryTextForCell:data] retain];
    _previewImage = [self.delegate previewIconForCell:data];//[[self.delegate previewIconForCell:data] retain];
    _fileURL = [self.delegate URLForCell:data];//[[self.delegate URLForCell:data] retain];
    
    // Wu:For the primaryText 02/05
    NSColor *primartTextColor = [self isHighlighted] ? [NSColor alternateSelectedControlTextColor] : 
    [NSColor purpleColor];//textColor
