...
- (id)initWithDelegate:(id)delegate userData:(long)userData
{
	if((self = [super init]))
	{
        ...
        theRunLoopModes = [NSArray arrayWithObject:NSDefaultRunLoopMode];//[[NSArray arrayWithObject:NSDefaultRunLoopMode] retain];
	}
	return self;
}
...
//	[theRunLoopModes release];
    theRunLoopModes = newRunLoopModes;//[newRunLoopModes retain];
...
- (BOOL)removeRunLoopMode:(NSString *)runLoopMode
{
    ...
    NSMutableArray *newRunLoopModes = [theRunLoopModes mutableCopy];//[[theRunLoopModes mutableCopy] autorelease];
...
- (NSArray *)runLoopModes
{
    ...
    return theRunLoopModes;//[[theRunLoopModes retain] autorelease];
}
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
        ...
        AsyncSocket *newSocket = [[[self class] alloc] initWithDelegate:theDelegate];//[[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
...
//        return (NSData *)NSMakeCollectable(peeraddr);//[(NSData *)NSMakeCollectable(peeraddr) autorelease];
	}
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
            ...
            theCurrentRead = [theReadQueue objectAtIndex:0];//[[theReadQueue objectAtIndex:0] retain];
...
- (void)maybeDequeueWrite
{
	...
	if((theCurrentWrite == nil) && (theWriteStream != NULL))
	{
		...
		{
			// Dequeue the next object in the write queue
            theCurrentWrite = [theWriteQueue objectAtIndex:0];//[[theWriteQueue objectAtIndex:0] retain];
...
//	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    ...
    AsyncSocket *theSocket = (__bridge AsyncSocket *)pInfo;//[[(__bridge AsyncSocket *)pInfo retain] autorelease];
...
//	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    ...
    AsyncSocket *theSocket = (__bridge AsyncSocket *)pInfo;//[[(AsyncSocket *)pInfo retain] autorelease];
