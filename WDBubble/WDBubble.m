...
#endif
    NSLog(@"NSURL URLWithRemoteChangedToLocal %@", storeURL.path);
    return storeURL;
}
...
#endif
    NSLog(@"NSURL URLWithoutNameConflict %@", storeURL.path);
    return storeURL;
}
...
- (void)readDataFromFile {
    if(!_streamDataBufferReader) {
        _streamDataBufferReader = [NSMutableData data];//[[NSMutableData data] retain];
    }
    uint8_t buf[1024];
    NSUInteger len = 0;
    ...
    if(len) {
        ...
        for (AsyncSocket *sock in _socketsConnect) {
            ...
            //NSLog(@"WDBubble NSStreamEventHasBytesAvailable wrote %@ to sock", [NSNumber numberWithInteger:len]);
        }
    } else {
        NSLog(@"WDBubble readDataFromFile will end with %@ sent", [NSNumber numberWithInteger:_streamBytesRead]);
        // DW: now we send these read data
        for (AsyncSocket *sock in _socketsConnect) {
...
//    [_streamDataBufferWriter release];
    ...
    if (_streamBytesWrote >= _currentMessage.fileSize) {
        ...
        NSLog(@"WDBubble writeDataToFile will end with %@ received", [NSNumber numberWithInteger:_streamBytesWrote]);
...
//            [_streamFileWriter release];
            ...
            NSLog(@"WDBubble writeDataToFile _streamFileWriter released");
        }
    }
}
...
- (void)publishServiceWithPassword:(NSString *)pwd {
    NSLog(@"WDBubble publishService <%@>%@ port %i", _service.name, _socketListen, _socketListen.localPort);
...
- (void)browseServices {
    ...
    // DW: set up browsers
    _browsers = [NSMutableArray arrayWithCapacity:kWDBubbleWebServiceTypeCount];//[[NSMutableArray arrayWithCapacity:kWDBubbleWebServiceTypeCount] retain];
    for (int i = 0; i < kWDBubbleWebServiceTypeCount; i++) {
        NSNetServiceBrowser *browser = [[NSNetServiceBrowser alloc] init];//[[[NSNetServiceBrowser alloc] init] autorelease];
...
- (void)broadcastMessage:(WDMessage *)message {
    _currentMessage = message;//[message retain];
...
- (void)terminateTransfer {
    NSLog(@"WDBubble terminateTransfer");
    if (_isReceiver) {
...
- (void)netService:(NSNetService *)sender didNotPublish:(NSDictionary *)errorDict {
    NSLog(@"NSNetServiceDelegate netService didNotPublish code: %@, domain: %@.",
...
- (void)onSocket:(AsyncSocket *)sock didAcceptNewSocket:(AsyncSocket *)newSocket {
    //NSLog(@"AsyncSocketDelegate didAcceptNewSocket %@: %@", sock, newSocket);
    ...
    _isReceiver = YES;
    _socketReceive = newSocket;//[newSocket retain];
    ...
    if ([_currentMessage.state isEqualToString:kWDMessageStateReadyToReceive]) {
        ...
        NSLog(@"WDBubble _streamFileWriter will be assigned path %@", _currentMessage.fileURL.path);
...
- (void)onSocket:(AsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag {
    //NSLog(@"AsyncSocketDelegate didReadData %@: %@", sock, data);
    ...
    if ([_currentMessage.state isEqualToString:kWDMessageStateReceiving]) {
        ...
        if (!_streamDataBufferWriter) {
            _streamDataBufferWriter = [NSMutableData data];//[[NSMutableData data] retain];
        }
...
- (void)onSocket:(AsyncSocket *)sock willDisconnectWithError:(NSError *)err {
    if (err) {
        NSLog(@"AsyncSocketDelegate willDisconnectWithError %@: %@", _currentMessage.state, err);
...
- (void)onSocket:(AsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port {
    ...
    if (_isReceiver) {
        ...
        //NSLog(@"AsyncSocketDelegate didConnectToHost writing %@", t);
#else   
        if ([_currentMessage.state isEqualToString:kWDMessageStateSending]) {
...
- (void)onSocket:(AsyncSocket *)sock didWriteDataWithTag:(long)tag {
    //NSLog(@"AsyncSocketDelegate didWriteDataWithTag %@", sock);
...
- (void)onSocketDidDisconnect:(AsyncSocket *)sock {
    //NSLog(@"AsyncSocketDelegate onSocketDidDisconnect %@", sock);
    if (_isReceiver) {
        // DW: a receive socket
        ...
        if ([_currentMessage.state isEqualToString:kWDMessageStateReceiving]) {
            ...
            if (_streamBytesWrote < _currentMessage.fileSize) {
                NSLog(@"WDBubble terminated receiver %@ received", [NSNumber numberWithInteger:_streamBytesWrote]);
                _streamBytesWrote = 0;
                [self closeStream:_streamFileWriter];
...
//            [_currentMessage release];
            ...
            if ([t.state isEqualToString: kWDMessageStateText]) {
                ...
                NSLog(@"WDBubble onSocketDidDisconnect %@ received kWDMessageStateReadyToSend", _currentMessage.state);
                // DW: begin of a file transfer
                _streamBytesWrote = 0;
                _currentMessage = [WDMessage messageWithFile:t.fileURL andState:kWDMessageStateReadyToReceive];//[[WDMessage messageWithFile:t.fileURL andState:kWDMessageStateReadyToReceive] retain];
                ...
                NSLog(@"WDBubble onSocketDidDisconnect %@ received kWDMessageStateReadyToReceive", _currentMessage.state);
...
//            [_dataBuffer release];
        ...
        if ([_currentMessage.state isEqualToString:kWDMessageStateSending]) {
            ...
            if (_streamBytesRead >= _currentMessage.fileSize) {
                ...
                NSLog(@"WDBubble terminated sender %@ sent", [NSNumber numberWithInteger:_streamBytesRead]);
                _streamBytesRead = 0;
                [self closeStream:_streamFileReader];
...
- (void)netServiceBrowser:(NSNetServiceBrowser *)netServiceBrowser didFindService:(NSNetService *)netService moreComing:(BOOL)moreServicesComing {
    ...
    NSLog(@"NSNetServiceBrowserDelegate didFindService %@", self.servicesFound);
...
- (void)netServiceBrowser:(NSNetServiceBrowser *)netServiceBrowser didFindDomain:(NSString *)domainName moreComing:(BOOL)moreDomainsComing {
    NSLog(@"NSNetServiceBrowserDelegate didFindDomain %@", domainName);
}
    ...
    switch(streamEvent) {
        case NSStreamEventEndEncountered: {
            if (_streamFileReader) {
                ...
                [self closeStream:_streamFileReader];
                NSLog(@"WDBubble NSStreamEventEndEncountered _streamFileReader");
            }
            
            if (_streamFileWriter) {
                // DW: receiver is complete
                [self closeStream:_streamFileWriter];
                NSLog(@"WDBubble NSStreamEventEndEncountered _streamFileWriter %@", theStream);
            }
            break;
        } case NSStreamEventErrorOccurred: {
            NSLog(@"WDBubble steam %@ NSStreamEventErrorOccurred %@", theStream, [theStream streamError]);
