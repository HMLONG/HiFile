...
//        }
        NSError* error;
        NSLog(@"%@",[message getFileURLofNStringPointerFormat]);
        //- (BOOL)removeItemAtURL:(NSURL *)URL error:(NSError **)error NS_AVAILABLE(10_6, 4_0);
        
//        [[NSFileManager defaultManager] removeItemAtURL:[message fileURL] error:&error];
//        
...
- (void)showHistoryPopOver:(NSView *)attachedView
{
    ...
    if (_historyPopOver == nil) {
        ...
        _historyPopOver.backgroundColor=[NSColor whiteColor];//[NSColor colorWithDeviceCyan:0.3 magenta:0.3 yellow:0.3 black:0.3 alpha:0.5];
...
- (TransparentTableView*)getFileHistoryTableView{
    return _fileHistoryTableView;
}

- (NSMutableArray *)getFileHistoryArray{
    return _fileHistoryArray;
}
