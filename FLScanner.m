...
{
    if (self = [super init]) {
        m_path = path;//[path retain];
        m_pi = progress;//[progress retain];
        m_display = display;//[display retain];
        m_error = nil;
        m_tree = nil;
...
- (BOOL) realScan
{
    ...
    dirstack = [[NSMutableArray alloc] init];//[[[NSMutableArray alloc] init] autorelease];
    dir = NULL;
